...
{
  UARTx->BR = (uint16_t)(RCCU_FrequencyValue(RCCU_FCLK)/(16*BaudRate));
}
...
* Return         : None
*******************************************************************************/
void UART_Config(UART_TypeDef *UARTx, uint32_t BaudRate, UARTParity_TypeDef Parity,
...
* Return         : None
*******************************************************************************/
void UART_ByteSend(UART_TypeDef *UARTx, uint8_t *Data)
{
...
* Return         : None
*******************************************************************************/
void UART_9BitByteSend(UART_TypeDef *UARTx, uint16_t *Data)
{
  if(UARTx->CR & (0x0001<<UART_FIFOEnableBit))// if FIFO ENABLED
    while((UARTx->SR & UART_TxFull)); // while the UART_TxFIFO contain 16 characters.
  else                  // if FIFO DISABLED
...
* Return         : None
*******************************************************************************/
void UART_DataSend(UART_TypeDef *UARTx, uint8_t *Data, uint8_t DataLength)
{
  while(DataLength--)
  {
...
* Return         : None
*******************************************************************************/
void UART_9BitDataSend(UART_TypeDef *UARTx, uint16_t *Data, uint8_t DataLength)
{
  while(DataLength--)
  {
...
* Return         : None
*******************************************************************************/
void UART_StringSend(UART_TypeDef *UARTx, uint8_t *String)
{
  uint8_t *Data=String;
  while(*Data != '\0')
    UART_ByteSend(UARTx, Data++);
...
*******************************************************************************/
uint16_t UART_ByteReceive(UART_TypeDef *UARTx, uint8_t *Data, uint8_t TimeOut)
{
   uint16_t wStatus = 0;
   ...
   *Data = (uint8_t)UARTx->RxBUFR; // then read the Receive Buffer Register
   return wStatus;
}
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
uint16_t UART_9BitByteReceive(UART_TypeDef *UARTx, uint16_t *Data, uint8_t TimeOut)
{
  uint16_t wStatus = 0;
  UARTx->TOR=TimeOut;// reload the Timeout counter
  while (!((wStatus=UARTx->SR) & (UART_TimeOutIdle|UART_RxHalfFull|UART_RxBufFull)));// while the UART_RxFIFO is empty and no Timeoutidle
  *Data = (uint16_t)UARTx->RxBUFR; // then read the RxBUFR
  return wStatus;
}
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
uint16_t UART_DataReceive(UART_TypeDef *UARTx, uint8_t *Data, uint8_t DataLength, uint8_t TimeOut)
{
  uint16_t wStatus = 0;
  while(DataLength--)
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
uint16_t UART_9BitDataReceive(UART_TypeDef *UARTx, uint16_t *Data, uint8_t DataLength, uint8_t TimeOut)
{
  uint16_t wStatus = 0;
  while(DataLength--)
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
uint16_t UART_StringReceive(UART_TypeDef *UARTx, uint8_t *Data)
{
  ...
  do
  {
    ...
    *(pSTRING++) = (uint8_t)UARTx->RxBUFR; // then read the RxBUFR
...
void sendchar( char *ch )
{
   ...
   UART_ByteSend(UARTx,(uint8_t *)ch);
}
