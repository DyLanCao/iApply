...
{
  uint32_t Tmp = ( RCCU->PLL1CR & ~RCCU_MX_Mask ) | ( New_Mul << RCCU_MX_Index );
...
{
  uint32_t Tmp = ( PCU->PLL2CR & ~RCCU_MX_Mask ) | ( New_Mul << RCCU_MX_Index );
...
* Return         : uint32_t
*******************************************************************************/
uint32_t RCCU_FrequencyValue ( RCCU_Clocks Internal_Clk )
{
  uint32_t Tmp;
  uint8_t Div = 0;
  uint8_t Mul = 0;
  ...
  Tmp = Tmp * Mul;
  /*return Tmp / Div */
  /*Div is in power of 2; 1,2,4,8,16,32,64,128, not 256 or above*/
  if(Div >= 128)
	  return Tmp >> 7;
  else if (Div >= 64)
	  return Tmp >> 6;
