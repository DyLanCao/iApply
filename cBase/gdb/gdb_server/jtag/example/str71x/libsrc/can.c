...
uint16_t CanTimings[] = {             // value   bitrate     NTQ  TSEG1  TSEG2  SJW  BRP
...
*******************************************************************************/
void CAN_SetBitrate(uint32_t bitrate)
{
...
* Note           : CAN must be in initialization mode
*******************************************************************************/
void CAN_SetTiming(uint32_t tseg1, uint32_t tseg2, uint32_t sjw, uint32_t brp)
{
...
* Return         : None
*******************************************************************************/
void CAN_SleepRequest(uint32_t WakeupMode)
{
	// Wakeup Line 6 is linked to CAN RX pin (port 1.11)
	// Wakeup Line 2 is linked to external pin (port 2.8)
	uint32_t WakeupLine = (WakeupMode == CAN_WAKEUP_ON_CAN ? XTI_Line6 : XTI_Line2);
...
*******************************************************************************/
static uint32_t CAN_GetFreeIF(void)
{
...
#define STD_RANGE_ID_MSK(range_start, range_end)	((uint16_t)((RANGE_ID_MSK((range_start), (range_end)) & 0x7FF) << 2))
#define STD_RANGE_ID_ARB(range_start, range_end)	((uint16_t)(RANGE_ID_ARB((range_start), (range_end)) << 2))

#define STD_FIXED_ID_MSK(id)	((uint16_t)((FIXED_ID_MSK(id) & 0x7FF) << 2))
#define STD_FIXED_ID_ARB(id)	((uint16_t)(FIXED_ID_ARB(id) << 2))

#define EXT_RANGE_ID_MSK_L(range_start, range_end)	((uint16_t)(RANGE_ID_MSK((range_start), (range_end)) >> 11))
#define EXT_RANGE_ID_MSK_H(range_start, range_end)	((uint16_t)(STD_RANGE_ID_MSK((range_start), (range_end)) | ((RANGE_ID_MSK((range_start), (range_end)) >> 27) & 0x03)))
#define EXT_RANGE_ID_ARB_L(range_start, range_end)	((uint16_t)(RANGE_ID_ARB((range_start), (range_end)) >> 11))
#define EXT_RANGE_ID_ARB_H(range_start, range_end)	((uint16_t)(STD_RANGE_ID_ARB((range_start), (range_end)) | ((RANGE_ID_ARB((range_start), (range_end)) >> 27) & 0x03)))

#define EXT_FIXED_ID_MSK_L(id)	((uint16_t)(FIXED_ID_MSK(id) >> 11))
#define EXT_FIXED_ID_MSK_H(id)	((uint16_t)(STD_FIXED_ID_MSK(id) | ((FIXED_ID_MSK(id) >> 27) & 0x03)))
#define EXT_FIXED_ID_ARB_L(id)	((uint16_t)(FIXED_ID_ARB(id) >> 11))
#define EXT_FIXED_ID_ARB_H(id)	((uint16_t)(STD_FIXED_ID_ARB(id) | ((FIXED_ID_ARB(id) >> 27) & 0x03)))
...
* Return         : None
*******************************************************************************/
void CAN_SetUnusedMsgObj(uint32_t msgobj)
{
	uint32_t msg_if;
...
* Return         : None
*******************************************************************************/
void CAN_SetTxMsgObj(uint32_t msgobj, uint32_t idType)
{
	uint32_t msg_if;
    while ((msg_if = CAN_GetFreeIF()) == 2);
...
* Return         : None
*******************************************************************************/
void CAN_SetRxMsgObj(uint32_t msgobj, uint32_t idType, uint32_t idLow, uint32_t idHigh, BoolStatus singleOrFifoLast)
{
	uint32_t msg_if;
    while ((msg_if = CAN_GetFreeIF()) == 2);
...
* Return         : None
*******************************************************************************/
void CAN_Init(uint8_t mask, uint32_t bitrate)
{
...
* Return         : None
*******************************************************************************/
void CAN_ReleaseMessage(uint32_t msgobj)
{
	uint32_t i;
...
*******************************************************************************/
uint32_t CAN_SendMessage(uint32_t msgobj, canmsg* pCanMsg)
{
...
//	return 0;	// message interface 0 not free
  ...
  CAN->sMsgObj[0].DA1R = ((uint16_t)pCanMsg->Data[1]<<8) | pCanMsg->Data[0];
  CAN->sMsgObj[0].DA2R = ((uint16_t)pCanMsg->Data[3]<<8) | pCanMsg->Data[2];
  CAN->sMsgObj[0].DB1R = ((uint16_t)pCanMsg->Data[5]<<8) | pCanMsg->Data[4];
  CAN->sMsgObj[0].DB2R = ((uint16_t)pCanMsg->Data[7]<<8) | pCanMsg->Data[6];
...
*******************************************************************************/
uint32_t CAN_ReceiveMessage(uint32_t msgobj, BoolStatus release, canmsg* pCanMsg)
{
    ...
    else
    {
      ...
      pCanMsg->Id = ((CAN->sMsgObj[1].A2R >> 2) & 0x07FF) | ((uint32_t)CAN->sMsgObj[1].A1R << 11) | (((uint32_t)CAN->sMsgObj[1].A2R & 0x0003) << 27);
    }

    pCanMsg->Dlc = CAN->sMsgObj[1].MCR & 0x0F;

    pCanMsg->Data[0] = (uint8_t) CAN->sMsgObj[1].DA1R;
    pCanMsg->Data[1] = (uint8_t)(CAN->sMsgObj[1].DA1R >> 8);
    pCanMsg->Data[2] = (uint8_t) CAN->sMsgObj[1].DA2R;
    pCanMsg->Data[3] = (uint8_t)(CAN->sMsgObj[1].DA2R >> 8);
    pCanMsg->Data[4] = (uint8_t) CAN->sMsgObj[1].DB1R;
    pCanMsg->Data[5] = (uint8_t)(CAN->sMsgObj[1].DB1R >> 8);
    pCanMsg->Data[6] = (uint8_t) CAN->sMsgObj[1].DB2R;
    pCanMsg->Data[7] = (uint8_t)(CAN->sMsgObj[1].DB2R >> 8);
...
*******************************************************************************/
uint32_t CAN_BasicSendMessage(canmsg* pCanMsg)
{
...
* Note           : CAN must be in BASIC mode
*******************************************************************************/
uint32_t CAN_BasicReceiveMessage(canmsg* pCanMsg)
{
